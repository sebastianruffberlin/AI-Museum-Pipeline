...
Emotion and Topics are optional and inactive in the `core` workflow. Emotion
is supplied as the museums-agnostic Machine Heart default and can be enabled
as-is (see below). Topics ships only a construction blueprint here -- its
taxonomy and curatorial framework are museum-specific and must be created by
the institution before Topics can be enabled.
