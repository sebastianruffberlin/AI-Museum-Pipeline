...
"""
    ...
    runner.ssh(
        deployment,
        f'chmod 600 "{gpu.config_dir}/runtime.env"',
    )

    if not skip_model_download:
        download_script = f"""
set -u
python3 "{gpu.install_path}/{gpu.deployment_path}/download-models.py" \
  --models-dir "{gpu.models_dir}"
"""
...
docker compose \
  --env-file "{gpu.config_dir}/runtime.env" \
